...
pub fn decode<P: AsRef<Path>>(path: P) -> Vec<Message> {
    ...
    unsafe {
        ...
        ffi::FitConvert_Init(
            &mut state as *mut ffi::FIT_CONVERT_STATE,
            FitBool::False as u8,
        );
    }
    ...
    loop {
        ...
        match reader.read(&mut buf) {
            Ok(0) => break,
            Ok(n) => unsafe {
                let return_val = ffi::FitConvert_Read(
                    &mut state as *mut ffi::FIT_CONVERT_STATE,
                ...
                match return_val {
                    ...
                    ffi::FIT_CONVERT_RETURN::FIT_CONVERT_PROTOCOL_VERSION_NOT_SUPPORTED => {
                        panic!("Protocol version not supported")
                    }
                    ffi::FIT_CONVERT_RETURN::FIT_CONVERT_DATA_TYPE_NOT_SUPPORTED => {
